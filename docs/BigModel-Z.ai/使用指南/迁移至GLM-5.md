...
<Tip>
  本文介绍如何将调用从 GLM-4.5 GLM-4.6 GLM-4.7 或其它早期模型迁移到我们迄今为止最强的编码模型 Z.ai GLM-5，涵盖采样参数差异、流式工具调用等要点。
</Tip>
...
* [ ] 最大输出与上下文：合理设置 `max_tokens`（GLM-5 最大输出 128K，上下文 200K）
...
* GLM-5 支持工具调用过程的实时流式构建与输出，默认 `False` 关闭，需同时打开：
