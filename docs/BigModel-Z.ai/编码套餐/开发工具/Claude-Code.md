...
* `ANTHROPIC_DEFAULT_HAIKU_MODEL`：`GLM-4.5-Air`
  ...
  GLM-5 参数量更大，对标 Claude Opus 模型，调用 GLM-5 时用量将按照 “高峰期 3 倍，非高峰期 2 倍” 进行计算抵扣；我们推荐您在复杂任务上切换至 GLM-5 处理，普通任务上继续使用 GLM-4.7，以避免套餐用量额度消耗过快。注：“高峰期”为每日的 14:00～18:00 （UTC+8）。
</Tip>
...
<Tabs>
  <Tab title="推荐安装方式">
    ...
    * MacOS 用户推荐使用 [nvm 方式安装 Nodejs](https://nodejs.org/en/download/) 或 [Homebrew 方式](https://formulae.brew.sh/formula/node)。不推荐直接安装包安装(后续可能会遇到权限问题)
...
<Note>
  **注意**：MacOS 用户若在安装过程中遇到权限问题，请使用 [nvm 方式安装 Nodejs](https://nodejs.org/en/download/)。\
  安装成功后，还需下方步骤二配置，先不要直接使用 `claude` 命令启动(可能由于网络或地区限制无法使用)。
</Note>
  ...
  <Step title="配置环境变量">
    ...
    <Tabs>
      ...
      <Tab title="方式三：手动配置">
        ...
        ```bash MacOS & Linux & Windows theme={null}
...
* `ANTHROPIC_DEFAULT_HAIKU_MODEL`：`GLM-4.5-Air`
  ...
  注：套餐已支持的用户使用 GLM-5，需要参考下方在`~/.claude/settings.json` 中，手动修改模型为 “glm-5”。
</Check>
...
1. 启动一个新的命令行窗口，运行`claude`启动 Claude Code，在 Claude Code 中输入`/status`确认模型状态：

![Description](https://cdn.bigmodel.cn/markdown/1759228558734cline-2.png?attname=cline-2.png)

### 视觉和搜索 MCP 服务器

参考 [视觉MCP服务器](/cn/coding-plan/mcp/vision-mcp-server) ，[搜索MCP服务器](/cn/coding-plan/mcp/search-mcp-server) 和 [网页读取MCP服务器](/cn/coding-plan/mcp/reader-mcp-server) 文档，配置完成后即可在 Claude Code 中使用。
