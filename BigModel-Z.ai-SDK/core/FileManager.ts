...
export class FileManager {
  ...
  async uploadFile(file: File): Promise<FileInfo> {
    ...
    const response = await fetch(`${this.client['baseUrl']}paas/v4/files/upload`, {
      method: 'POST',
      headers: {
  ...
  async webSearch(query: string, maxResults = 10): Promise<any> {
  ...
  async webRead(url: string): Promise<any> {
