...
export class KnowledgeBaseManager {
  ...
  async uploadDocument(
    ...
    const response = await fetch(
      `${this.client['baseUrl']}paas/v4/knowledge-bases/${kbId}/documents`,
      {
        method: 'POST',
        headers: {
  ...
  async uploadDocumentFromUrl(
    ...
    const response = await this.client.request<{
      ...
      `paas/v4/knowledge-bases/${kbId}/documents/url`,
      {
        method: 'POST',
  ...
  async revectorize(kbId: string, docId: string): Promise<void> {
    await this.client.request(
      `paas/v4/knowledge-bases/${kbId}/documents/${docId}/revectorize`,
      {
        method: 'POST',
      },
    )
  ...
  async getUsage(kbId: string): Promise<any> {
    const response = await this.client.request('paas/v4/knowledge-bases/' + kbId + '/usage', {
      method: 'GET',
    })
    return response,
  }
