...
# @file test-agents-fixed.sh
# @description YYC³ AI-Family 七大智能体测试脚本（修复版，减少误报）
# @author YYC³ Team
# @version 1.0.1
# @created 2026-02-26
# @tags [testing],[agents],[fixed]
...
test_agent_configs() {
  ...
  for agent in "${agents[@]}"; do
    ...
    if [ -d "$PROJECT_ROOT/src/lib" ]; then
      if find "$PROJECT_ROOT/src/lib" -name "*${agent}*" -o -name "*${agent,,}*" 2>/dev/null | grep -q .; then
        found=1
      fi
    fi
    ...
    if [ -d "$PROJECT_ROOT/packages/bigmodel-sdk" ]; then
      if find "$PROJECT_ROOT/packages/bigmodel-sdk" -name "*${agent}*" -o -name "*${agent,,}*" 2>/dev/null | grep -q .; then
        found=1
      fi
    fi
    ...
    if [ $found -eq 1 ]; then
      ...
      echo -e "${YELLOW}⚠️  WARN${NC} | $agent 配置（未找到，可能使用默认配置）"
      WARN=$((WARN + 1))
    fi
  done
}
